...
### Changed
- `hooks/session-start` — auto-detects bootstrap state on every session start; if `CLAUDE.md` still contains `[Project Name]` placeholder, Claude offers Flow A (inception) or Flow B (project-adoption) before any other work; if already bootstrapped, injects live repo state as before
